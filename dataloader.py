...
class EndoVis18VQAClassification(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        visual_feature_loc = os.path.join(loc[0],loc[1],loc[2], '/content/drive/MyDrive/vision_language_project/Surgical-VQLA/dataset/EndoVis-18-VQLA/seq_1/vqla/img_features', (str(self.patch_size)+'x'+str(self.patch_size)),loc[-1].split('_')[0]+'.hdf5')
...
class EndoVis17VQAClassificationValidation(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # img
        loc = self.vqas[idx][0].split('/')
        visual_feature_loc = os.path.join(loc[0],loc[1], '/content/drive/MyDrive/vision_language_project/Surgical-VQLA/dataset/EndoVis-17-VQLA/vqla/img_features', (str(self.patch_size)+'x'+str(self.patch_size)),loc[-1].split('.')[0]+'.hdf5')
        frame_data = h5py.File(visual_feature_loc, 'r')    
        visual_features = torch.from_numpy(frame_data['visual_features'][:])
